...
- **Parallel tasks, one extension.** Every MCP process gets its own tab group, and one process can run several tasks by passing a stable `session_id` on its browser calls. A task cannot drive or adopt another task's tabs, and cleanup (including a process exiting) closes only its own. On macOS any number of MCP processes share the one extension; on Windows and Linux one process owns it at a time, so run parallel tasks through one server with `session_id`. Tasks share Chrome's cookies and logins, and desktop apps and the clipboard are not isolated.
