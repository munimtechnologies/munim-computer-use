...
//!
//! Matches `macos/Sources/AgentCursor.swift`:
...
//! idle breathe. No click ring. Disabled with `COMPUTER_USE_AGENT_CURSOR=0`.
//!
...
/// another tools/call starts. Override with `COMPUTER_USE_AGENT_CURSOR_TASK_FADE_SECS`.
...
fn ensure_fade_watcher() {
    ...
    let _ = thread::Builder::new()
        .name("munim-agent-cursor-fade".into())
        .spawn(|| {
            loop {
